$(document).ready(function() {
    ...
    function validateForm() {
        ...
        if (!tenantName) {
            $('#tenantName').removeClass('border-gray-300').addClass('border-red-500');
        ...
        if (cmReading < pmReading) {
            $('#cmReading').removeClass('border-gray-300').addClass('border-red-500');
            errorMessages.push('Current reading cannot be less than previous reading');
            isValid = false;
        }
    ...
    function generateBill() {
        ...
        showNotification(`Bill generated successfully for ${tenantName}!`, 'success');
    ...
    $('#resetButton').click(function(event) {
        ...
        if ($('#billReport tr').length > 0) {
            if (!confirm('Are you sure you want to reset the form? This will clear all generated bills.')) {
                return;
            }
        }
    ...
    $('.form-input').on('focus', function() {
        $(this).parent().addClass('transform scale-105 transition-transform duration-200');
    }).on('blur', function() {
